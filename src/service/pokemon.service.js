...
class PokemonService {
    ...
    async findPokemonByTrainer(nameTrainer) {
        const treinador = await this.trainerService.findNameTrainer(nameTrainer);
        const treinadorId = treinador.id;

        // Realiza a consulta para obter os nomes dos pokémons
        const pokemons = await this.db("pokemon").where({ treinadorId }).pluck('nome');

        // Retorna somente a lista de nomes dos pokémons
        return pokemons;
    }
